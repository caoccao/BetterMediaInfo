...
async function buildCaptureArgs(mode: CaptureMode, p: CaptureParams, opts: OutputOptions, file: string): Promise<string[]> {
  const ext = opts.format === 'jpg' ? 'jpg' : 'png';
  const stem = await getStem(file);
  const outPattern = await join(opts.outputDir, `${stem}_shot_%04d.${ext}`);
  const args: string[] = [];
...
async function buildTimestampArgs(file: string, opts: OutputOptions, timeSec: number, index: number): Promise<string[]> {
  const ext = opts.format === 'jpg' ? 'jpg' : 'png';
  const stem = await getStem(file);
  const out = await join(opts.outputDir, `${stem}_ts_${String(index + 1).padStart(4, '0')}.${ext}`);
  const args: string[] = [];
  if (opts.hwaccelEnabled && opts.hwaccel) {
    args.push('-hwaccel', opts.hwaccel);
  }
...
function FfmpegTools({ file }: FfmpegToolsProps) {
  ...
  // While capturing, the preview slider doubles as the progress bar: drive it
  // from the capture progress instead of the seek position.
  useEffect(() => {
    if (capturing && durationSeconds > 0) {
      setPosition((progress / 100) * durationSeconds);
    }
  }, [progress, capturing, durationSeconds]);
  ...
  useEffect(() => {
    ...
    const unlistenProgress = w.listen<Protocol.FfmpegCaptureProgress>('ffmpeg-capture-progress', (event) => {
      const { percent, done, cancelled, error } = event.payload;
      setProgress(percent);
  ...
  // Single right-aligned line under the slider. While capturing, the slider
  // doubles as the progress bar so this shows the percentage instead of the
  // video metadata: "{percent}% · {current} / {total}".
  let sliderInfo: string;
  if (capturing) {
    sliderInfo = `${progress}% · ${formatTime(position)} / ${formatTime(durationSeconds)}`;
  } else {
      ...
      >
        ...
        <Box sx={{ px: 2, pt: 1, pb: 2, flexShrink: 0, height: SLIDER_AREA_HEIGHT, boxSizing: 'border-box' }}>
          <Slider
            ...
            step={0.1}
            disabled={capturing || metaLoading || durationSeconds <= 0}
            valueLabelDisplay="auto"
